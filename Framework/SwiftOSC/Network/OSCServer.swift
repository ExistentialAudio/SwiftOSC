...
//                let typeEnd = messageData.firstIndex(of: 0x00)!
                guard let typeEnd = messageData.firstIndex(of: 0x00) else {
                    func buildCustomMessage() -> OSCMessage {
                        ...
                        customMessage.add(String("\(name ?? String(describing: self.listener?.port)) OSC exception_ *** dirty fix 'type end found nil' *** (TotalMix bundle data with missing type end)"))
